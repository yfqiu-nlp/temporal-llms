...
for domain in domains:
    ...
    for ans in original_predictions:
        if re.search(r'\bno\b', ans) and re.search(r'\byes\b', ans):
            # both yes and no are in the answer
            start_yes, end_yes = re.search(r'\byes\b', ans).span()
            start_no, end_no = re.search(r'\bno\b', ans).span()
            if start_yes < start_no:
                original_temp.append('yes')
            else:
                original_temp.append('no')
        elif re.search(r'\bno\b', ans):
            ...
            original_temp.append('')
    
    for ans in negative_predictions:
        if re.search(r'\bno\b', ans) and re.search(r'\byes\b', ans):
            # both yes and no are in the answer
            start_yes, end_yes = re.search(r'\byes\b', ans).span()
            start_no, end_no = re.search(r'\bno\b', ans).span()
            if start_yes < start_no:
                negative_temp.append('yes')
            else:
                negative_temp.append('no')
        elif re.search(r'\bno\b', ans):
            # only no is in the answer
            negative_temp.append('no')
        elif re.search(r'\byes\b', ans):
            ...
            # nothing in the answer
            negative_temp.append('')
    # print(original_temp)
    
    original_predictions = original_temp
    negative_predictions = negative_temp
